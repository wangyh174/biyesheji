...
| 项目 | 值 |
|:---|:---|
| **做什么** | 用 CLIP 做质量过滤与语义一致性审查，但避免在后处理阶段再次强化性别刻板偏差 |
| **CLIP 文本** | 默认使用以职业为中心的中性文本（如 doctor / nurse），而不是直接复用生成 prompt |
| **淘汰规则** | 去除 CLIP 分过低、明显不像真实人物、或更像 toy / object / deformed / low-quality 的样本 |
| **语义约束** | 保留 group margin 与 human-photo 约束，但默认不强制 `pred_group == group`，避免过度筛除 Fair-Diffusion 已控制过的样本 |
| **精选规则** | 先温和过滤，再在每个 `group x y_true` 内做排序与等量保留，形成最终平衡集 |
| **为什么** | 目标不是再次“纠正性别”，而是在不破坏 Fair-Diffusion 生成分布的前提下，做保守的质量对齐 |
| **输出** | `data/metadata_balanced.csv`（后续所有实验的唯一数据源） |

> **提示**: Stage 02 在当前主实验中采用“公平敏感版 CLIP 后处理”。
> 它不再简单等价于“原始 prompt 匹配分 Top-K”，而是“温和过滤 + 组内排序 + 组间等量保留”。
