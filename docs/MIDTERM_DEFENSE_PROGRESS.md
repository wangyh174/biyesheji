...
当前版本已不再使用“手工特征 + LogisticRegression”的旧代理实现，而是改为面向公开预训练权重的推理框架。当前优先纳入的目标检测器包括：
...
同时，F3Net / Gram 仍保留为扩展检测器接入方向，但不再作为当前阶段的默认主检测器组合。
...
在实际运行中，已率先跑通 `CNNDetection` 检测与公平性评估链路，并已将 `UnivFD`、`DIRE`、`LGrad` 接入当前检测脚本接口。
...
2. 新增了 `UnivFD`、`DIRE`、`LGrad` 的检测器接口及本地权重路径支持，扩展了主流 AIGC 检测器覆盖范围。
...
- `UnivFD`、`DIRE` 与 `LGrad` 已完成脚本级接入，可进入后续权重准备与实跑阶段
...
2. 完整跑通 `UnivFD`、`DIRE` 与 `LGrad` 的稳定推理流程，并视时间决定是否继续补充 F3Net / Gram。
