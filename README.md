...
3.  **Stage 03 & 04: 基准评估 (Baseline)** - 运行 CNNDetection, F3Net, LGrad 计算 FPR Gap 和 Fairness Metrics。
...
执行以下单条命令即可运行完整毕设流程：
```bash
python scripts/00_run_local_pipeline.py --real-source local --detectors cnndetection,f3net
```
